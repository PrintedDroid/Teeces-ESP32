/**
 * ===============================================================================================
 * Printed Droid Teeces Logic Display Controller - ESP32-S3 Mini Version 4.6
 * ===============================================================================================
 *
 ...
 *
 * WIRING (ESP32-S3 Mini):
 * Front Chain (TFLD + BFLD + PSI):  DATA=GPIO1, CLK=GPIO3, CS=GPIO2
 * Rear Chain (RLD + Rear PSI):      DATA=GPIO4, CLK=GPIO6, CS=GPIO5
 ...
 *
 * NEW IN v4.6:
 * - Corrected pin assignments for actual S3 Mini board wiring
 ...
 *
 * ADDRESSES:
 * 0 = All displays      4 = Front PSI      6 = Front Holo [v5.0]
 * 1 = Top FLD           5 = Rear PSI       7 = Rear Holo  [v5.0]
 * 2 = Bottom FLD                           8 = Top Holo   [v5.0]
 * 3 = Rear LD
 *
 ...
 *
 * H COMMANDS (HOLOPROJECTOR CONTROL) [NEW v5.0]:
 ...
 *
 * * COMMANDS (MARCDUINO HOLO PROTOCOL) [NEW v5.0]:
...
// Pin Definitions (corrected for actual S3 Mini wiring v4.6)
...
// Holoprojector NeoPixel Strips (NEW v5.0)
...
// Holoprojector functions (NEW v5.0)
...
void setup() {
  ...
  Serial.println(F("\n╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║   ESP32-S3 Mini Teeces v4.6 - R2-D2 Controller           ║"));
  Serial.println(F("║   + 3 Holoprojector NeoPixel Outputs                    ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝"));
  Serial.println(F(""));
  Serial.println(F("v4.6 Changes:"));
  Serial.println(F("  Corrected pin assignments for S3 Mini wiring"));
  Serial.println(F("  Serial baud rate: 9600 (JawaLite standard)"));
  Serial.println(F("  💡 3 Holoprojector outputs (Front=GP11, Rear=GP12, Top=GP13)"));
  Serial.println(F("  ⭐ MarcDuino *ON/*OF holo commands supported"));
  Serial.println(F("  🎨 H-Command for direct holo control (6H1=on, 0H0=off)"));
  Serial.println(F("  🌈 Holo effects: solid, rainbow, flicker"));
  Serial.println(F("  🔧 ESP32-S3 Mini platform"));
  Serial.println(F(""));
  ...
  // Initialize Holoprojectors (NEW v5.0)
...
void loop() {
  ...
  if (Serial.available()) {
    char ch = Serial.peek();
    if (ch == '*') {
      // v5.0: Check if this is a MarcDuino holo command (*ON, *OF, *ST, *RD)
    ...
    // v5.0: FlthyHP protocol — commands starting with A/F/R/T/S followed by digits
...
void runNormalOperation() {
  ...
  // Update Holoprojector animations (NEW v5.0)
...
void parseCommand(char* inputStr) {
  ...
  if (strcmp(inputStr, "?" "?") == 0 || strcmp(inputStr, "help") == 0) {
    ...
    Serial.println(F("           6=FrontHolo, 7=RearHolo, 8=TopHolo [v5.0]\n"));
    ...
    Serial.println(F("  H[arg]  - Holo (0=off, 1=on, 2=color, 3=rainbow, 4=flicker) [v5.0]"));
    Serial.println(F("  D       - Dimension command\n"));
    Serial.println(F("MarcDuino Holo commands [v5.0]:"));
  ...
  switch (command) {
    ...
    case 'L':
      ...
      break;
    case 'H':
      // NEW v5.0: H-Command for holoprojector control
      if (hasArgument) {
        doHcommand(address, argument);
      } else {
        Serial.println(F("\nERROR: H command requires argument (0=off, 1=on, 2=color, 3=rainbow, 4=flicker)"));
        Serial.write(0x7);
      }
      break;
    default:
      ...
      Serial.println(F("'. Valid: T/M/P/R/S/L/C/D/H. Type '?" "?' for help."));
      Serial.write(0x7);
      break;
  }
...
void runDiagnostics() {
  ...
  Serial.print(F("[ 2/8 ] Rear Logic Display (GPIO4/6/5)... "));
  for (int dev = 0; dev < 3; dev++) {
  ...
  Serial.println(F("  • RLD not working? Check GPIO 4/6/5 connections"));
...
// =======================================================================================
// HOLOPROJECTOR CONTROL (NEW v5.0)
// =======================================================================================
...
void doHcommand(int address, int argument) {
    HoloState state = (HoloState)constrain(argument, 0, 4);
