# Teeces ESP32 Logic Display Controller v4.3
...
### Version 4.3 (2026-06-19)

**Bug Fixes & Documentation Corrections**

#### 🔧 Fixes
- Added missing forward declarations for `doCcommand` / `doLcommand` (build no longer relies on Arduino auto-prototyping)
- Added address validation (0–5) with a clear error message for out-of-range addresses
- `M`-Command text is now sanitized like boot text (non-printable bytes filtered)
- **Digital PSI brightness now works**: the 0–15 brightness setting (incl. `L4`/`L5`) is applied to the NeoPixel strips — at boot and on every change — and now **defaults to 10/15** (previously digital PSIs always ran at full brightness)
- Watchdog timer is now reset during hardware diagnostics; removed unnecessary blocking delays in the setup wizard
- `buildCommand` input buffer is reset when entering/leaving the config menu (prevents stray half-commands)
...
#### 🎨 Code Quality
- Removed misleading color aliases (`BLUE`/`YELLOW` and `RED`/`GREEN` had collided to the same PSI color slots); the code now uses `PSI_COLOR1`/`PSI_COLOR2` directly
- Replaced magic numbers (PSI pattern length, color-delay unit) with named constants
...
#### ⚠️ Breaking Changes
None. All existing commands behave as before.
