/**
 * ===============================================================================================
 * Printed Droid Teeces Logic Display Controller - ESP32-S3 Zero Version 4.7
 * ===============================================================================================
 *
 ...
 * - Three NeoPixel Holoprojector outputs (Front, Rear, Top).
 *
 * WIRING (ESP32-S3 Zero):
 * Front Chain (TFLD + BFLD + PSI):  DATA=GPIO1, CLK=GPIO3, CS=GPIO2
 * Rear Chain (RLD + Rear PSI):      DATA=GPIO4, CLK=GPIO5, CS=GPIO6  (CLK/CS swapped for SN74AHCT125 wiring)
 * Digital PSI 1 (configurable as Front/Rear): DATA=GPIO7
 * Digital PSI 2 (configurable as Front/Rear): DATA=GPIO8
 * I2C SDA (reserved):    GPIO9
 * I2C SCL (reserved):    GPIO10
 * Front Holo (NeoPixel): DATA=GPIO11
 * Rear Holo (NeoPixel):  DATA=GPIO12
 * Top Holo (NeoPixel):   DATA=GPIO13
 * Serial RX/TX:          Board-labeled RX/TX pins (9600 baud)
 *
 *
 * ===============================================================================================
 ...
 *
 * NEW IN v4.7:
 * - Bugfix: Holo state name array out-of-bounds (states 5-8)
 * - H-Command now supports full state range 0-8
 * - HOLO_COLOR keeps last color instead of forcing white
 * - FlthyHP fallback no longer swallows JawaLite commands
 * - Watchdog reset added to boot sequence & diagnostics
 * - Address validation (0-8) with error feedback
 * - Corrected color palette documentation
 *
 * NEW IN v4.6:
 * - Corrected pin assignments for actual S3 Zero board wiring
 * - Serial baud rate corrected to 9600 (JawaLite standard)
 * - 💡 3 Holoprojector NeoPixel outputs (Front=GPIO11, Rear=GPIO12, Top=GPIO13)
 * - ⭐ MarcDuino * commands for Holo control (*ON00, *OF00, *ON01-03, *OF01-03, *ST00)
 * - 🎨 H-Command: Direct Holo control (6H1=FrontHolo on, 0H0=all off)
 * - ⚙️ Configurable LEDs per Holo (default: 7 NeoPixels each)
 * - 🌈 Holo LED effects: solid color, rainbow cycle, color chase
 * - ESP32-S3 Zero platform
 * ===============================================================================================
 ...
 *
 * ADDRESSES:
 * 0 = All displays      4 = Front PSI      6 = Front Holo [v4.6]
 * 1 = Top FLD           5 = Rear PSI       7 = Rear Holo  [v4.6]
 * 2 = Bottom FLD                           8 = Top Holo   [v4.6]
 * 3 = Rear LD
 *
 ...
 * Color palette: 0=RED, 1=GREEN, 2=BLUE, 3=YELLOW, 4=CYAN, 5=MAGENTA,
 *                6=ORANGE, 7=PURPLE, 8=PINK, 9=LIME, 10=SKYBLUE, 11=WHITE
 *
 * H COMMANDS (HOLOPROJECTOR CONTROL) [NEW v4.6]:
 * H[arg] = Holo state (0=off, 1=on/white, 2=color, 3=rainbow, 4=flicker)
 * Addresses: 0=all holos, 6=Front, 7=Rear, 8=Top
 * Examples: 0H1 = all holos on, 6H0 = front holo off, 7H3 = rear holo rainbow
 *
 * * COMMANDS (MARCDUINO HOLO PROTOCOL) [NEW v4.6]:
 * *ON00 = All holos on       *OF00 = All holos off
 * *ON01 = Front holo on      *OF01 = Front holo off
 * *ON02 = Rear holo on       *OF02 = Rear holo off
 * *ON03 = Top holo on        *OF03 = Top holo off
 * *ST00 = All holos reset    *RD = Random holo flicker
 *
 *
 * ===============================================================================================
...
#warning "Enable 'USB CDC On Boot' - GPIO1/GPIO3 drive the front MAX7219 chain; serial runs over native USB."
#endif
...
// Pin Definitions (corrected for actual S3 Zero wiring v4.6)
#define FRONT_DATA_PIN  1
#define FRONT_CLK_PIN   3
#define FRONT_CS_PIN    2
#define REAR_DATA_PIN   4
// NOTE: Rear CLK/CS are swapped vs. the "standard" Teeces pinout to match this
// build's SN74AHCT125 level-shifter wiring (GP5 -> MAX7219 CLK, GP6 -> LOAD/CS).
#define REAR_CLK_PIN    5
#define REAR_CS_PIN     6
#define DIGITAL_PSI1_PIN 7
#define DIGITAL_PSI2_PIN 8

// I2C Pins (active on board, currently reserved)
#define SDA_PIN         9
#define SCL_PIN         10

// Holoprojector Pins
#define HOLO_FRONT_PIN  11
#define HOLO_REAR_PIN   12
#define HOLO_TOP_PIN    13
#define HOLO_LED_COUNT  7       // LEDs per holoprojector (configurable)
#define NUM_HOLOS       3
...
enum HoloState {
    HOLO_OFF = 0, HOLO_ON = 1, HOLO_COLOR = 2, HOLO_RAINBOW = 3, HOLO_FLICKER = 4,
    HOLO_LEIA = 5, HOLO_COLOR_CYCLE = 6, HOLO_DIM_PULSE = 7, HOLO_SHORT_CIRCUIT = 8
};
...
// Bit reversal lookup
const uint8_t revlookup[16] PROGMEM = {0x0,0x8,0x4,0xC,0x2,0xA,0x6,0xE,0x1,0x9,0x5,0xD,0x3,0xB,0x7,0xF};
...
// Holoprojector NeoPixel Strips (NEW v4.6)
Adafruit_NeoPixel holoFront(HOLO_LED_COUNT, HOLO_FRONT_PIN, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel holoRear(HOLO_LED_COUNT, HOLO_REAR_PIN, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel holoTop(HOLO_LED_COUNT, HOLO_TOP_PIN, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel* holoStrips[NUM_HOLOS] = { &holoFront, &holoRear, &holoTop };
HoloState holoStates[NUM_HOLOS] = { HOLO_OFF, HOLO_OFF, HOLO_OFF };
uint32_t holoColors[NUM_HOLOS] = { 0xFFFFFF, 0xFFFFFF, 0xFFFFFF };  // Default: white
unsigned long holoAnimTimers[NUM_HOLOS] = { 0, 0, 0 };
byte holoAnimStep[NUM_HOLOS] = { 0, 0, 0 };
...
// Holoprojector functions (NEW v4.6)
void setHolo(byte holoIdx, HoloState state, uint32_t color = 0xFFFFFF);
void setAllHolos(HoloState state, uint32_t color = 0xFFFFFF);
void updateHoloAnimations();
void doHcommand(int address, int argument);
void parseMarcDuinoHoloCmd(char* inputStr);
void parseFlthyHPCommand(char* cmd);
void parseFlthyMasterSeq(int seqNum);
void updateHoloAutoTwitch();
...
void setup() {
  Serial.begin(9600);  // JawaLite standard baud rate
  ...
  Serial.println(F("\n╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║   ESP32-S3 Zero Teeces v4.7 - R2-D2 Controller           ║"));
  Serial.println(F("║   + 3 Holoprojector NeoPixel Outputs                    ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝"));
  Serial.println(F(""));
  Serial.println(F("v4.7 Changes:"));
  Serial.println(F("  🐛 Fixed holo state name array out-of-bounds (states 5-8)"));
  Serial.println(F("  🎨 H-Command full state range 0-8; HOLO_COLOR keeps last color"));
  Serial.println(F("  🔧 FlthyHP fallback no longer swallows JawaLite commands"));
  Serial.println(F("  🛡 Watchdog reset in boot sequence & diagnostics"));
  Serial.println(F("  ✓ Address validation (0-8) with error feedback"));
  Serial.println(F(""));

  // Check if this is first-time setup
  firstTimeSetup = !preferences.getBool("setupDone", false);

  // One-time migration: on the first boot after installing this firmware, force the
  // PSI output to digital (the default carrier-board wiring) so a leftover analog
  // value in flash can't leave the digital PSIs dark. Runs exactly once — a later
  // deliberate "set psi_output 0" (analog) is preserved across reboots.
  if (!preferences.getBool("psiDefaulted", false)) {
    preferences.putUChar("psiOutput", PSI_DIGITAL);
    preferences.putBool("psiDefaulted", true);
  }
  ...
  if (globalPsiOutput == PSI_DIGITAL) {
    ...
    Serial.println(F("✓ Digital PSIs activated (GPIO 7 & 8)."));
  } else {
    setFPSI(PSI_COLOR1);
    setRPSI(PSI_COLOR2);
  ...
  // Initialize Holoprojectors (NEW v4.6)
  for (int i = 0; i < NUM_HOLOS; i++) {
    holoStrips[i]->begin();
    holoStrips[i]->setBrightness(150);
    holoStrips[i]->clear();
    holoStrips[i]->show();
  }
  Serial.println(F("✓ Holoprojectors initialized (GPIO 11/12/13)."));
...
void loop() {
  ...
  if (Serial.available()) {
    char ch = Serial.peek();
    if (ch == '*') {
      // v4.6: Check if this is a MarcDuino holo command (*ON, *OF, *ST, *RD)
      // or a config menu entry (standalone *)
      Serial.read();  // consume the *
      if (Serial.available()) {
        // More data follows — likely a MarcDuino holo command
        static char holoBuf[12];
        holoBuf[0] = '*';
        int hIdx = 1;
        delay(5);  // Small delay to collect rest of command
        while (Serial.available() && hIdx < 11) {
          char c = Serial.read();
          if (c == '\r' || c == '\n') break;
          holoBuf[hIdx++] = c;
        }
        holoBuf[hIdx] = '\0';
        parseMarcDuinoHoloCmd(holoBuf);
      } else {
        // Standalone * — enter config menu
        configMode = true;
        printConfigMenu();
      }
    }
    // v4.6: FlthyHP protocol — commands starting with A/F/R/T/S followed by digits
    else if (ch == 'A' || ch == 'F' || ch == 'R' || ch == 'T' || ch == 'S') {
      static char flthyBuf[16];
      int fIdx = 0;
      delay(5);  // Small delay to collect full command
      while (Serial.available() && fIdx < 14) {
        char c = Serial.read();
        if (c == '\r' || c == '\n') break;
        flthyBuf[fIdx++] = c;
      }
      flthyBuf[fIdx] = '\0';
      // Verify it looks like a FlthyHP command (letter + digits)
      if (fIdx >= 2 && flthyBuf[1] >= '0' && flthyBuf[1] <= '9') {
        parseFlthyHPCommand(flthyBuf);
      } else {
        // Not FlthyHP — pass through to normal JawaLite parser.
        // Feed the buffered characters, then a '\r' terminator so the
        // command is actually built and executed (the terminator consumed
        // by the read loop above must be reissued here).
        for (int i = 0; i < fIdx; i++) {
          if (buildCommand(flthyBuf[i], cmdString)) {
            parseCommand(cmdString);
          }
        }
        if (buildCommand('\r', cmdString)) {
          parseCommand(cmdString);
        }
      }
    }
...
void runNormalOperation() {
  ...
  // Update Holoprojector animations (NEW v4.6)
  updateHoloAnimations();
  updateHoloAutoTwitch();
}
...
void loadProfileDefaults(byte profileNum, ConfigSettings &cfg) {
  ...
  cfg.rldBrightness = 3; cfg.rpsiBrightness = 10;
  cfg.fldBrightness = 3; cfg.fpsiBrightness = 10;
...
void applyBrightnessSettings() {
  ...
  // Digital PSI strips: apply the same 0-15 brightness, scaled to the 0-255 NeoPixel
  // range, so the brightness setting also works in digital mode (applied at boot and
  // on every L-command). The main loop redraws the strips with the new scale.
  if (globalPsiOutput == PSI_DIGITAL) {
    PsiConfig psiCfg = getPsiConfig();
    psiCfg.frontStrip->setBrightness(map(currentSettings.fpsiBrightness, 0, 15, 0, 255));
    psiCfg.rearStrip->setBrightness(map(currentSettings.rpsiBrightness, 0, 15, 0, 255));
  }
}
...
void drawLetter(byte display, char let, int shift) {
  ...
  for (byte i = 0; i < 5; i++) {
    if (shift >= 0)
      LEDgrid[display][i] |= ((unsigned long)letterBitmap[i]) << shift;
    else
      LEDgrid[display][i] |= ((unsigned long)letterBitmap[i]) >> -shift;
  }
}
...
void parseCommand(char* inputStr) {
  ...
  if (strcmp(inputStr, "?" "?") == 0 || strcmp(inputStr, "help") == 0) {
    ...
    Serial.println(F("           6=FrontHolo, 7=RearHolo, 8=TopHolo [v4.6]\n"));
    ...
    Serial.println(F("  L[arg]  - Brightness (0-15) [v4.2]"));
    Serial.println(F("  C[p][c] - PSI color: pattern(1-2) + color(0-11) [v4.2]"));
    Serial.println(F("  H[arg]  - Holo (0=off, 1=on, 2=color, 3=rainbow, 4=flicker) [v4.6]"));
    Serial.println(F("  D       - Dimension command\n"));
    Serial.println(F("MarcDuino Holo commands [v4.6]:"));
    Serial.println(F("  *ON00-03  Holo on (0=all, 1=front, 2=rear, 3=top)"));
    Serial.println(F("  *OF00-03  Holo off"));
    Serial.println(F("  *ST00     Reset all holos"));
    Serial.println(F("  *RD       Random flicker\n"));
    ...
    Serial.println(F("  6H3      Front holo rainbow"));
    Serial.println(F("  0H1      All holos on\n"));
  ...
  if (!isdigit((unsigned char)inputStr[0])) {
  ...
  char addrStr[3];
  if (!isdigit((unsigned char)inputStr[pos])) {
  ...
  if (pos < length && isdigit((unsigned char)inputStr[pos])) {
  ...
  // Validation: address range (0=all, 1-3 displays, 4-5 PSI, 6-8 holos)
  if (address > 8) {
    Serial.println(F("\nERROR: Address must be 0-8"));
    Serial.write(0x7);
    return;
  }
  ...
  if (pos >= length) {
    Serial.println(F("\nERROR: Missing command character (T/M/P/R/S/L/C/D/H)"));
    Serial.write(0x7);
    return;
  }
  ...
  switch (command) {
    ...
    case 'L':
      ...
      break;
    case 'H':
      // NEW v4.6: H-Command for holoprojector control
      if (hasArgument) {
        doHcommand(address, argument);
      } else {
        Serial.println(F("\nERROR: H command requires argument (0=off,1=on,2=color,3=rainbow,4=flicker,5=leia,6=cycle,7=pulse,8=short)"));
        Serial.write(0x7);
      }
      break;
    default:
      ...
      Serial.println(F("'. Valid: T/M/P/R/S/L/C/H. Type '?" "?' for help."));
      Serial.write(0x7);
      break;
  }
...
void runDiagnostics() {
  ...
  Serial.print(F("[ 2/8 ] Rear Logic Display (GPIO4/5/6)... "));
  for (int dev = 0; dev < 3; dev++) {
  ...
  Serial.print(F("[ 3/8 ] Front Logic Displays (GPIO1/3/2)... "));
  ...
  Serial.print(F("[ 6/8 ] Digital PSI 1 (GPIO7)... "));
  digitalPsi1.clear();
  ...
  Serial.print(F("[ 7/8 ] Digital PSI 2 (GPIO8)... "));
  digitalPsi2.clear();
  ...
  Serial.println(F("  • RLD not working? Check GPIO 4/5/6 connections"));
  Serial.println(F("  • FLD not working? Check GPIO 1/3/2 connections"));
  Serial.println(F("  • Digital PSI issues? Check GPIO 7/8 and 5V power"));
...
void runSetupWizard() {
  Serial.println(F("\n╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║         WELCOME TO TEECES v4.7 - SETUP WIZARD           ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝\n"));
...
void printConfigMenu() {
  Serial.println(F("\n"));
  Serial.println(F("╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║         TEECES v4.7 - CONFIGURATION MENU                ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝"));
  Serial.println(F(""));
...
void printFullHelp() {
  Serial.println(F("\n"));
  Serial.println(F("══════════════════════════════════════════════════════════"));
  Serial.println(F("                 TEECES v4.7 QUICK START"));
  Serial.println(F("══════════════════════════════════════════════════════════"));
  Serial.println(F(""));
...
// =======================================================================================
// HOLOPROJECTOR CONTROL (NEW v4.6)
// =======================================================================================
// 3 NeoPixel holoprojector outputs: Front (GPIO11), Rear (GPIO12), Top (GPIO13)
// Controlled via:
//   - JawaLite H-Command: [addr]H[state]  (addr: 0=all, 6=front, 7=rear, 8=top)
//   - MarcDuino * commands: *ON00-03, *OF00-03, *ST00, *RD
// States: 0=off, 1=on(white), 2=color, 3=rainbow, 4=flicker

// Set a single holo to a state
void setHolo(byte holoIdx, HoloState state, uint32_t color) {
    if (holoIdx >= NUM_HOLOS) return;

    holoStates[holoIdx] = state;
    holoColors[holoIdx] = color;
    holoAnimTimers[holoIdx] = millis();
    holoAnimStep[holoIdx] = 0;

    Adafruit_NeoPixel* strip = holoStrips[holoIdx];
    ...
    switch (state) {
        case HOLO_OFF:
            ...
            break;
        case HOLO_ON:
            for (int i = 0; i < HOLO_LED_COUNT; i++) {
        ...
        case HOLO_RAINBOW:
        case HOLO_FLICKER:
            // Handled in updateHoloAnimations()
            break;
    }

    const char* holoNames[] = {"Front", "Rear", "Top"};
    const char* stateNames[] = {"OFF", "ON", "COLOR", "RAINBOW", "FLICKER",
                                "LEIA", "COLOR_CYCLE", "DIM_PULSE", "SHORT_CIRCUIT"};
    if (verboseMode) {
        Serial.print(F("[HOLO] "));
        Serial.print(holoNames[holoIdx]);
        Serial.print(F(" → "));
        if (state <= HOLO_SHORT_CIRCUIT) Serial.println(stateNames[state]);
        else Serial.println(state);
    }
}

// Set all holos to the same state
void setAllHolos(HoloState state, uint32_t color) {
    for (byte i = 0; i < NUM_HOLOS; i++) {
        setHolo(i, state, color);
    }
}

// Non-blocking holo animation update — call from loop()
void updateHoloAnimations() {
    unsigned long now = millis();

    for (byte i = 0; i < NUM_HOLOS; i++) {
        Adafruit_NeoPixel* strip = holoStrips[i];

        if (holoStates[i] == HOLO_RAINBOW) {
            // Rainbow cycle: rotate hue every 30ms
            if (now - holoAnimTimers[i] > 30) {
                holoAnimTimers[i] = now;
                holoAnimStep[i]++;
                for (int p = 0; p < HOLO_LED_COUNT; p++) {
                    uint16_t hue = ((uint16_t)holoAnimStep[i] * 256 + p * 65536 / HOLO_LED_COUNT) * 256;
                    strip->setPixelColor(p, strip->gamma32(strip->ColorHSV(hue)));
                }
                strip->show();
            }
        }
        else if (holoStates[i] == HOLO_FLICKER) {
            // Random flicker: randomize brightness every 50-150ms
            if (now - holoAnimTimers[i] > (50 + random(100))) {
                holoAnimTimers[i] = now;
                for (int p = 0; p < HOLO_LED_COUNT; p++) {
                    byte bright = random(80, 255);
                    // Extract base color components and scale
                    byte r = (holoColors[i] >> 16) & 0xFF;
                    byte g = (holoColors[i] >> 8) & 0xFF;
                    byte b = holoColors[i] & 0xFF;
                    strip->setPixelColor(p, (r * bright / 255), (g * bright / 255), (b * bright / 255));
                }
                strip->show();
            }
        }
        else if (holoStates[i] == HOLO_LEIA) {
            // Leia: white pulsing (smooth sine-wave brightness)
            if (now - holoAnimTimers[i] > 20) {
                holoAnimTimers[i] = now;
                holoAnimStep[i]++;
                byte bright = (byte)(128 + 127 * sin(holoAnimStep[i] * 0.05));
                for (int p = 0; p < HOLO_LED_COUNT; p++) {
                    strip->setPixelColor(p, bright, bright, bright);
                }
                strip->show();
            }
        }
        else if (holoStates[i] == HOLO_COLOR_CYCLE) {
            // Color cycle: slow full-spectrum rotation (all LEDs same color)
            if (now - holoAnimTimers[i] > 50) {
                holoAnimTimers[i] = now;
                holoAnimStep[i]++;
                uint16_t hue = (uint16_t)holoAnimStep[i] * 256;
        ...
        else if (holoStates[i] == HOLO_DIM_PULSE) {
            // Dim pulse: colored breathing effect
            if (now - holoAnimTimers[i] > 30) {
                holoAnimTimers[i] = now;
                holoAnimStep[i]++;
                float scale = 0.3 + 0.7 * (0.5 + 0.5 * sin(holoAnimStep[i] * 0.04));
                byte r = (byte)(((holoColors[i] >> 16) & 0xFF) * scale);
                byte g = (byte)(((holoColors[i] >> 8) & 0xFF) * scale);
                byte b = (byte)((holoColors[i] & 0xFF) * scale);
                for (int p = 0; p < HOLO_LED_COUNT; p++) {
        ...
        else if (holoStates[i] == HOLO_SHORT_CIRCUIT) {
            // Short circuit: rapid white flashes with random blue sparks
            if (now - holoAnimTimers[i] > (20 + random(60))) {
                holoAnimTimers[i] = now;
                holoAnimStep[i]++;
                bool flash = (holoAnimStep[i] % 3 == 0);
                for (int p = 0; p < HOLO_LED_COUNT; p++) {
                    if (flash) {
                        strip->setPixelColor(p, 255, 255, 255);
                    } else if (random(4) == 0) {
                        strip->setPixelColor(p, random(50), random(100), 255); // blue sparks
                    } else {
                ...
                if (holoAnimStep[i] > 80) {
                    setHolo(i, HOLO_OFF);
                }
            }
        }
        // HOLO_OFF, HOLO_ON, HOLO_COLOR are static — no animation needed
    }
}

// H-Command handler: [addr]H[state]
// addr: 0=all holos, 6=front, 7=rear, 8=top
// state: 0=off, 1=on(white), 2=color(last color), 3=rainbow, 4=flicker
void doHcommand(int address, int argument) {
    HoloState state = (HoloState)constrain(argument, 0, 8);
    ...
    if (address == 0) {
        // All holos
        if (state == HOLO_COLOR) {
            // Preserve each holo's previously stored color instead of forcing white
            for (byte i = 0; i < NUM_HOLOS; i++) {
                setHolo(i, state, holoColors[i]);
            }
        } else {
            setAllHolos(state);
        }
        ...
        byte idx = address - 6;
        if (state == HOLO_COLOR) {
            // Preserve the stored color instead of forcing white
            setHolo(idx, state, holoColors[idx]);
        } else {
            setHolo(idx, state);
        }
        const char* names[] = {"Front", "Rear", "Top"};
        Serial.print(F("\n✓ "));
        Serial.print(names[idx]);
        Serial.print(F(" holo: "));
    } else {
        Serial.println(F("\nERROR: H command addresses: 0=all, 6=front, 7=rear, 8=top"));
        Serial.write(0x7);
        return;
    }
...
// Parse MarcDuino holo commands: *ON00, *OF00, *ON01-03, *OF01-03, *ST00, *RD
void parseMarcDuinoHoloCmd(char* inputStr) {
    if (strlen(inputStr) < 4) return;  // Minimum: *RD\0
    ...
    // *ON00 - *ON03: Turn on holo(s)
    if (strncmp(inputStr, "*ON", 3) == 0) {
        int num = atoi(inputStr + 3);
        if (num == 0) {
            setAllHolos(HOLO_ON);
            Serial.println(F("\n✓ All holos ON"));
        } else if (num >= 1 && num <= 3) {
            setHolo(num - 1, HOLO_ON);
            const char* names[] = {"Front", "Rear", "Top"};
            Serial.print(F("\n✓ "));
            Serial.print(names[num - 1]);
            Serial.println(F(" holo ON"));
        }
    }
    // *OF00 - *OF03: Turn off holo(s)
    else if (strncmp(inputStr, "*OF", 3) == 0) {
        int num = atoi(inputStr + 3);
        if (num == 0) {
            setAllHolos(HOLO_OFF);
            Serial.println(F("\n✓ All holos OFF"));
        } else if (num >= 1 && num <= 3) {
            setHolo(num - 1, HOLO_OFF);
            const char* names[] = {"Front", "Rear", "Top"};
            Serial.print(F("\n✓ "));
            Serial.print(names[num - 1]);
            Serial.println(F(" holo OFF"));
        }
    }
    // *ST00: Stop/Reset all holos
    else if (strncmp(inputStr, "*ST", 3) == 0) {
        setAllHolos(HOLO_OFF);
        Serial.println(F("\n✓ All holos RESET"));
    }
    // *RD: Random holo flicker (all)
    else if (strncmp(inputStr, "*RD", 3) == 0) {
        setAllHolos(HOLO_FLICKER);
        Serial.println(F("\n✓ All holos FLICKER"));
    }
    else {
        if (verboseMode) {
            Serial.print(F("\n[HOLO] Unknown * command: "));
            Serial.println(inputStr);
        }
    }
}
...
// =======================================================================================
// FlthyHP PROTOCOL SUPPORT
// =======================================================================================
// Format: [Target][SeqNum][Params]
// Target: A=All, F=Front, R=Rear, T=Top
// SeqNum: 001-007=LED effects, 096=clear, 097=auto, 101-199=servo(ignored)
// Params: C=color(0-9), S=speed(ignored)
//
// Supported LED sequences:
//   D001 = Leia (white pulsing)          D005 = Toggle On/Off
//   D002C = Color Projector              D006 = Rainbow
//   D003CS = Dim Pulse                   D007 = Short Circuit
//   D004C = Color Cycle                  D096 = Clear/Off
//   D0971 = Auto+Default                 D0972 = Auto+Random
//
// Master sequences (S prefix):
//   S1 = Leia, S4 = Clear, S5 = Auto Enable, S9 = Random

// FlthyHP color palette (matches FlthyHP firmware)
static const uint32_t flthyColors[] = {
...
#define FLTHY_COLOR_COUNT 10

bool holoAutoTwitch = false;  // Auto-twitch mode (random flicker in idle)
unsigned long holoAutoTimer = 0;
...
void parseFlthyHPCommand(char* cmd) {
    ...
    // Determine which holo(s) to control
    // A=All, F=Front(0), R=Rear(1), T=Top(2)
    int holoIdx = -1;  // -1 = all
    switch (target) {
        case 'A': holoIdx = -1; break;
        case 'F': holoIdx = 0;  break;
        case 'R': holoIdx = 1;  break;
        case 'T': holoIdx = 2;  break;
        case 'S':  // Master sequence
            parseFlthyMasterSeq(atoi(seqStr));
            return;
        default:
            if (verboseMode) { Serial.print(F("\n[FLTHY] Unknown target: ")); Serial.println(target); }
            return;
    }

    // Extract sequence number (first 3 digits or until non-digit)
    int seqNum = 0;
    int i = 0;
    while (seqStr[i] >= '0' && seqStr[i] <= '9' && i < 4) {
        seqNum = seqNum * 10 + (seqStr[i] - '0');
        i++;
    }

    // Extract optional color parameter (after 'C')
    uint32_t color = 0xFFFFFF;  // Default white
    char* colorPtr = strchr(seqStr, 'C');
    if (colorPtr && colorPtr[1] >= '0' && colorPtr[1] <= '9') {
        int cIdx = colorPtr[1] - '0';
        if (cIdx < FLTHY_COLOR_COUNT) color = flthyColors[cIdx];
    }

    if (verboseMode) {
        Serial.print(F("\n[FLTHY] Target=")); Serial.print(target);
        Serial.print(F(" Seq=")); Serial.print(seqNum);
        Serial.print(F(" Color=0x")); Serial.println(color, HEX);
    }

    // Map FlthyHP sequence to HoloState
    HoloState state = HOLO_OFF;
    switch (seqNum) {
        case 1:   state = HOLO_LEIA;          color = 0xFFFFFF; break;  // Leia: always white
        case 2:   state = HOLO_COLOR;          break;  // Color projector
        case 3:   state = HOLO_DIM_PULSE;      break;  // Dim pulse
        case 4:   state = HOLO_COLOR_CYCLE;    break;  // Color cycle
        case 5:   state = HOLO_ON;             break;  // Toggle on/off
        case 6:   state = HOLO_RAINBOW;        break;  // Rainbow
        case 7:   state = HOLO_SHORT_CIRCUIT;  break;  // Short circuit
        case 96:  state = HOLO_OFF;            break;  // Clear
        case 97:  // Auto mode
            holoAutoTwitch = true;
            holoAutoTimer = millis();
            Serial.println(F("\n✓ Holo auto-twitch ON"));
            return;
        default:
            // 100+ range = servo commands → ignored (no servo hardware)
            if (seqNum >= 100) {
                if (verboseMode) { Serial.print(F("\n[FLTHY] Servo cmd ignored: ")); Serial.println(seqNum); }
                return;
            }
            if (verboseMode) { Serial.print(F("\n[FLTHY] Unknown seq: ")); Serial.println(seqNum); }
            return;
    }

    // Special: seq 5 = toggle
    if (seqNum == 5) {
        byte checkIdx = (holoIdx >= 0) ? holoIdx : 0;
        if (holoStates[checkIdx] != HOLO_OFF) {
            state = HOLO_OFF;
        } else {
            state = HOLO_ON;
            color = 0xFFFFFF;
    ...
    // Apply to target holo(s)
    if (holoIdx < 0) {
        setAllHolos(state, color);
    } else {
        setHolo(holoIdx, state, color);
    }
    holoAutoTwitch = false;  // Manual command cancels auto
}

void parseFlthyMasterSeq(int seqNum) {
    switch (seqNum) {
        case 1:  // Leia: all holos white pulse
            setAllHolos(HOLO_LEIA, 0xFFFFFF);
            Serial.println(F("\n✓ FlthyHP Master S1: Leia"));
            break;
        case 4:  // Clear all
            setAllHolos(HOLO_OFF);
            holoAutoTwitch = false;
            Serial.println(F("\n✓ FlthyHP Master S4: Clear all"));
            break;
        case 5:  // Auto enable
            holoAutoTwitch = true;
            holoAutoTimer = millis();
            Serial.println(F("\n✓ FlthyHP Master S5: Auto-twitch ON"));
            break;
        case 9:  // Random: random effect on random holo
            {
                byte rHolo = random(NUM_HOLOS);
                HoloState rStates[] = { HOLO_ON, HOLO_RAINBOW, HOLO_FLICKER, HOLO_COLOR_CYCLE };
                setHolo(rHolo, rStates[random(4)], flthyColors[random(FLTHY_COLOR_COUNT)]);
                Serial.println(F("\n✓ FlthyHP Master S9: Random"));
            }
            break;
        default:
            if (verboseMode) { Serial.print(F("\n[FLTHY] Unknown master seq: ")); Serial.println(seqNum); }
            break;
    }
}

// Auto-twitch: random holo activity when idle (call from loop)
void updateHoloAutoTwitch() {
    if (!holoAutoTwitch) return;
    unsigned long now = millis();
    if (now - holoAutoTimer > (3000 + random(8000))) {  // 3-11 second intervals
        holoAutoTimer = now;
        byte rHolo = random(NUM_HOLOS);
        if (random(3) == 0) {
            // Sometimes turn one off
            setHolo(rHolo, HOLO_OFF);
        } else {
            // Random effect with random color
            HoloState effects[] = { HOLO_ON, HOLO_FLICKER, HOLO_RAINBOW, HOLO_DIM_PULSE, HOLO_COLOR };
            setHolo(rHolo, effects[random(5)], flthyColors[random(FLTHY_COLOR_COUNT)]);
        }
    }
}