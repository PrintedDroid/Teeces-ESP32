/**
 * ===============================================================================================
 * Printed Droid Teeces Logic Display Controller - ESP32 Enhanced Version 4.3
 * ===============================================================================================
 *
 ...
 *
 * NEW IN v4.3:
 * - 🛠 Added missing forward-declarations for doCcommand / doLcommand
 * - ✓ Address validation in parseCommand (rejects addresses > 5 with error + BEL)
 * - 🔒 M-Command text is now sanitized (sanitizeText) like boot texts
 * - 💡 L4/L5 now also set digital NeoPixel PSI brightness (not only analog MAX7219)
 * - 🐶 Diagnostics: watchdog (WDT) reset between every step
 * - ⏱ Setup wizard: cosmetic delays reduced (1000ms -> 200ms)
 * - 🧹 buildCommand input buffer/position reset on entering/leaving config mode
 * - 🎨 Removed misleading color aliases (BLUE/RED/YELLOW/GREEN); use PSI_COLOR1/2
 * - 📝 Corrected color palette documentation to match actual colorPalette[]
 *
 * NEW IN v4.2:
...
// NOTE: Misleading color aliases (BLUE/RED/YELLOW/GREEN) were removed in v4.3.
// They mapped only to the two analog PSI states PSI_COLOR1 / PSI_COLOR2 and were
// confusing (YELLOW==BLUE, GREEN==RED). Use PSI_COLOR1 / PSI_COLOR2 directly.
...
// Number of LED indices in each digital PSI pattern array above
#define PSI_PATTERN_LEN (sizeof(psi_pattern1_indices) / sizeof(psi_pattern1_indices[0]))
// Time unit (ms) multiplied by nextColorDelay to schedule the next PSI color change
#define PSI_COLOR_DELAY_UNIT_MS 500
...
// Write position into cmdString for buildCommand(). File-global (was function-static)
// so it can be reset when entering/leaving config mode to avoid stale half-commands.
uint8_t buildCommandPos = 0;
...
void setup() {
  ...
  Serial.println(F("\n╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║     ESP32 Teeces v4.3 - Enhanced R2-D2 Controller       ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝"));
  Serial.println(F(""));
  Serial.println(F("NEW in v4.3:"));
  Serial.println(F("  💡 L4/L5 now also dim digital PSI strips"));
  Serial.println(F("  🔒 M-command text is sanitized like boot texts"));
  Serial.println(F("  ✓ Address validation (0-5) in JawaLite parser"));
  Serial.println(F("  🧹 Config-mode entry/exit clears stale command buffer"));
  Serial.println(F("  🎨 Removed misleading color aliases (use PSI_COLOR1/2)"));
  Serial.println(F(""));
...
void loop() {
  ...
  if (Serial.available()) { 
    ...
    if (ch == '*') {
      Serial.read();
      configMode = true;
      // Reset any half-typed JawaLite command so it can't leak into config mode
      buildCommandPos = 0;
      cmdString[0] = '\0';
      printConfigMenu();
    }
...
void setDigitalPsi(Adafruit_NeoPixel &strip, PsiState mode, byte color1_idx, byte color2_idx) {
  ...
  switch (mode) {
    case PSI_TEST:
      ...
      break;
    case PSI_COLOR1:
      for (uint8_t i = 0; i < PSI_PATTERN_LEN; i++) {
...
void randomDigitalPsi(PsiRandomState<Adafruit_NeoPixel> &state, byte color1_idx, byte color2_idx, bool getsStuck) {
  ...
  if (state.isStuck == 1) {
    ...
    if (currentMillis - state.lastColorChange > state.nextColorDelay * PSI_COLOR_DELAY_UNIT_MS) {
      state.lastColorChange = currentMillis;
  ...
  if (state.currentPattern == 1 && row < HPROW) {
    ...
    for (uint8_t i = 0; i < rowLen; i++) {
      ...
      bool is_p2 = false;
      for (uint8_t p = 0; p < PSI_PATTERN_LEN; p++) {
...
byte buildCommand(char ch, char* output_str) {
  // Uses file-global buildCommandPos so the buffer can be reset on config-mode transitions
  if (ch == '\r') {
    output_str[buildCommandPos] = '\0';
    buildCommandPos = 0;
    return true;
  } else {
    if (buildCommandPos < CMD_MAX_LENGTH - 1) {
      output_str[buildCommandPos] = ch;
      buildCommandPos++;
    } else {
...
void parseCommand(char* inputStr) {
  ...
  // Validation: address must be in valid range (0-5)
  if (address > 5) {
    Serial.println(F("ERROR: Address must be 0-5"));
    Serial.write(0x7);  // BEL - audible alert
    return;
  }
...
void doMcommand(int address, char* message) {
  // Sanitize incoming text (strip non-printable chars) just like boot texts
...
void doLcommand(int address, int argument) {
  ...
  if (address == 0) {
    ...
    // Address 1: Top FLD
    // NOTE: TFLD (L1) and BFLD (L2) share a single fldBrightness setting,
    // so setting one also affects the other FLD.
    currentSettings.fldBrightness = argument;
    Serial.print(F(" [Top FLD]"));
  }
  else if (address == 2) {
    // Address 2: Bottom FLD
    // NOTE: BFLD (L2) shares fldBrightness with TFLD (L1) - see above.
    currentSettings.fldBrightness = argument;
    ...
    // Address 4: Front PSI (analog intensity; digital handled below)
    currentSettings.fpsiBrightness = argument;
    Serial.print(F(" [Front PSI]"));
  }
  else if (address == 5) {
    // Address 5: Rear PSI (analog intensity; digital handled below)
    currentSettings.rpsiBrightness = argument;
    Serial.print(F(" [Rear PSI]"));
  }

  // Apply the new brightness settings immediately (analog MAX7219 intensity)
  applyBrightnessSettings();

  // applyBrightnessSettings() above also applies the digital PSI strip brightness
  // (front = fpsiBrightness, rear = rpsiBrightness) when in digital mode.
}
...
void runSetupWizard() {
  Serial.println(F("\n╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║         WELCOME TO TEECES v4.3 - SETUP WIZARD           ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝\n"));
...
void printConfigMenu() {
  Serial.println(F("\n"));
  Serial.println(F("╔══════════════════════════════════════════════════════════╗"));
  Serial.println(F("║         TEECES v4.3 - CONFIGURATION MENU                ║"));
  Serial.println(F("╚══════════════════════════════════════════════════════════╝"));
  Serial.println(F(""));
...
void printFullHelp() {
  Serial.println(F("\n"));
  Serial.println(F("══════════════════════════════════════════════════════════"));
  Serial.println(F("                 TEECES v4.3 QUICK START"));
  Serial.println(F("══════════════════════════════════════════════════════════"));
  Serial.println(F(""));
  ...
  if (Serial.available()) {
    ...
    // NEW v4.2: Shorthand commands
      ...
      if (hasUnsavedChanges) {
        ...
        configMode = false;
        // Reset command buffer so no stale half-command carries into normal mode
        buildCommandPos = 0;
        cmdString[0] = '\0';
