# Teeces ESP32 Logic Display Controller v4.7
**Advanced ESP32-S3 Zero based Teeces display controller for R2-D2 logic displays, PSI indicators, and holoprojectors**
...
### Version 4.7 (2026-06-19)

**Bug Fixes & Documentation Corrections**

#### Fixes
- Fixed out-of-bounds read on holo state-name arrays for states 5–8 (Leia, Color Cycle, Dim Pulse, Short Circuit) in verbose mode
- H-Command now exposes the full state range **0–8** (previously clamped to 0–4; states 5–8 were only reachable via FlthyHP)
- `H2` (Color) now keeps the last set color instead of forcing white
- FlthyHP fallback no longer swallows valid JawaLite commands (missing carriage return)
- Watchdog timer is now reset during the boot sequence and hardware diagnostics (prevents spurious reboots)
- Added address validation (0–8) with a clear error message for out-of-range addresses
- Added a compile-time reminder that "USB CDC On Boot" must be enabled (GPIO1/GPIO3 drive the front MAX7219 chain)
- One-time migration: forces the PSI output to **digital** on the first boot of this firmware (prevents dark PSIs caused by a leftover analog value in flash); a later deliberate analog choice is preserved
...
#### Documentation
- Corrected the **color palette** table — it now matches the firmware (RED, GREEN, BLUE, YELLOW, CYAN, MAGENTA, ORANGE, PURPLE, PINK, LIME, SKYBLUE, WHITE)
- Unified version strings across boot message, config menu and wizard

#### Breaking Changes
None. All existing commands behave as before; the H-Command simply accepts more states.
...
- Upgraded from ESP32-C3 Mini to ESP32-S3 Zero
- Corrected all pin assignments to match actual S3 Zero board wiring
...
- **Color Palette**: 12 colors (RED, GREEN, BLUE, YELLOW, CYAN, MAGENTA, ORANGE, PURPLE, PINK, LIME, SKYBLUE, WHITE)
...
| 5 | Rear Chain CLK | MAX7219 SPI |
| 6 | Rear Chain CS | MAX7219 SPI |
...
2. **Board Configuration for ESP32-S3 Zero:**
   - Board: "ESP32S3 Dev Module" (or "Waveshare ESP32-S3-Zero" in newer cores)
   - Upload Speed: "921600"
...
4C111  → Front PSI, Pattern 1 = White (11)
5C28   → Rear PSI, Pattern 2 = Pink (8)
4C14   → Front PSI, Pattern 1 = Cyan (4)
...
- Check MAX7219 wiring: Front (DATA=GPIO1, CLK=GPIO3, CS=GPIO2), Rear (DATA=GPIO4, CLK=GPIO5, CS=GPIO6)
...
- **Hardware Compatibility**: Printed Droid Teeces32 Boards (ESP32-S3 Zero, MAX7219, NeoPixel)
